    ...
    <div class="row g-3">
                        ...
                        <table class="table table-hover align-middle mb-0" id="dispatchTable">
                            ...
                            <tbody>
                                ...
                                <tr class="dispatch-row" data-incident="<?php echo e($incident?->incident_number ?? ''); ?>" data-vehicle="<?php echo e($vehicleName); ?>" data-status="<?php echo e($status); ?>" data-assigned="<?php echo e($assignedIso); ?>">
                                    <td>
                                        <div class="fw-semibold"><?php echo e($incident?->incident_number ?? 'N/A'); ?></div>
                                        <div class="small text-muted"><?php echo e($incident?->location ?? 'No location'); ?></div>
                                    </td>
                                    <td><?php echo e($vehicleName); ?></td>
                                    <td>
                                        <span class="badge rounded-pill <?php echo e($statusClass); ?>"><?php echo e(str_replace('_',' ', ucfirst($status))); ?></span>
                                    </td>
                                    <td><?php echo e($assigned?->format('M d, Y H:i') ?? '—'); ?></td>
                                    <td><?php echo e($accepted?->format('M d, Y H:i') ?? '—'); ?></td>
                                    <td>
                                        <?php if($responseMinutes !== null): ?>
                                        <span class="fw-semibold"><?php echo e($responseMinutes); ?> min</span>
                                        <?php else: ?>
                                        —
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo e($completed?->format('M d, Y H:i') ?? '—'); ?></td>
                                </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                        </table>
        ...
        <div class="col-lg-4">
            <div id="timelineView" class="card border-0 shadow-sm rounded-4 p-3">
                ...
                <?php else: ?>
                <div class="timeline">
                    <?php $__currentLoopData = $dispatches->sortByDesc(fn($d)=> $d->assigned_at); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dispatch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                    $incident = $dispatch->incident;
                    $vehicleName = $dispatch->vehicle?->vehicle_name ?? $dispatch->ambulance?->vehicle_name ?? 'N/A';
                    $assigned = $dispatch->assigned_at;
                    ...
                    <div class="timeline-item d-flex gap-3 mb-3">
                        <div class="timeline-marker">
                            <span class="rounded-circle d-inline-block <?php echo e($dotClass); ?>" style="width:14px;height:14px;border:3px solid #fff;"></span>
                        </div>
                        <div class="flex-grow-1">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <div class="fw-semibold"><?php echo e($incident?->incident_number ?? 'N/A'); ?> — <?php echo e($vehicleName); ?></div>
                                    <div class="small text-muted"><?php echo e($incident?->location ?? 'No location'); ?></div>
                                </div>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.driver', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\muniresq-project\resources\views/driver/history.blade.php ENDPATH**/ ?>